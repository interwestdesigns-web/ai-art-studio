...
def api():
    ...
    if action == 'generate-prompts':
        resp = requests.post('https://api.anthropic.com/v1/messages', headers={'Content-Type': 'application/json', 'x-api-key': CLAUDE_KEY, 'anthropic-version': '2023-06-01'}, json={'model': 'claude-opus-4-1', 'max_tokens': 1024, 'messages': [{'role': 'user', 'content': f"Generate 4 wall art prompts. User: {data.get('description')}. Ratio: {data.get('ratio')}. Format: 4 prompts one per line."}]}, timeout=30)
        ...
        for prompt in data.get('prompts', []):
            try:
                resp = requests.post(f"https://queue.fal.run/{data.get('model')}", headers={'Authorization': f"Key {FAL_KEY}", 'Content-Type': 'application/json'}, json={'prompt': prompt, 'image_size': 'landscape', 'num_inference_steps': 30}, timeout=60)
